...
const Modal = (props) => {
    ...
    const fetchEvent = async (id) => {
        return new Promise((resolve, reject) => {
            axios.get(`${baseUrl}/events/${id.eventId}`, {headers: authHeader()}).then(async response => {
    ...
    useEffect(async () => {
        const slots = await Promise.all([fetchSlot(props.data.reservationSlotId)])
        const events = await Promise.all([fetchEvent(slots[0])])
        setData(events[0])
    }, [])
    ...
    return (
        <div className={styles.modalWindow} onClick={props.onClose}>
            <div className={styles.contentBody} onClick={e => e.stopPropagation()}>
                <div className={styles.flex}>
                    <div className={styles.EventDescription}>
                        ...
                        <div className={styles.description}>
                            <p><strong>Source: </strong>{props.data.sources[0]}</p>
                        </div>
                        <div className={styles.description}>
                            <p>
                                <strong>Address: </strong>{data.address.city}, {data.address.street}, {data.address.houseNumber}
                            </p>
                        </div>
                        <div className={styles.description}>
                            <p><strong>Date: </strong>{props.data.date}</p>
                        </div>
                        <div className={styles.description}>
                            <p><strong>From Time: </strong>{props.data.fromTime}</p><p><strong>To
                            Time: </strong>{props.data.toTime}</p>
                        </div>
                        <div className={styles.description}>
                            <p><strong>Pricing: </strong>{props.data.pricing}</p>
                        </div>
                    </div>
                    <div className={styles.ClientDescription}>
                        <h3>Customer Details:</h3>
                        <div className={styles.description}>
                            <p><strong>Firstname: </strong>{props.data.userFirstName}</p>
                            <p><strong>Lastname: </strong>{props.data.userLastName}</p>
                            <p><strong>Username: </strong>{props.data.username}</p>
                            <p><strong>Email: </strong>{props.data.userEmail}</p>
                        </div>
                    </div>
                </div>
                <div className={styles.btns}>
                    <button className={'button-primary'} type={"button"} onClick={props.onClose}>Close</button>
                </div>
            </div>
        </div>
    )
